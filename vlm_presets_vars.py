...
*   **Timeline Segment and Camera Continuity:** Timestamp blocks divide time, action, sound, or foreground priority; they are not shot declarations. Never add `[Shot N]` markers merely because a new timestamp block begins. Treat the video as one continuous camera take by default. When tracking, panning, orbiting, zooming, or another camera movement crosses a timestamp boundary, explicitly state that the same movement continues rather than restarting it. Introduce a cut or transition only when `{user_query}` explicitly schedules one or when the requested scene necessarily changes location or viewpoint discontinuously. Describe a genuine cut or transition naturally inside [VISUAL] without assigning a `[Shot N]` label. Neither the phrases \"first shot\" or \"opening shot\" nor a requested segment count authorizes one shot per segment. They establish the opening visual state or timing structure unless `{user_query}` explicitly requests a multi-shot structure or named cuts.
