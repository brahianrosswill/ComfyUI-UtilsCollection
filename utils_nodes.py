    ...
    def execute(cls, topic: str) -> io.NodeOutput:
        topics = {
            "node_catalog": (
                ...
                "- `UC_AdvancedMiniMaxH3ImageToVideo`: complete MiniMax H3 first-frame, first/last-frame, or image-reference conditioning. It coordinates Qwen visual sources with positional VAE keyframes or ordered `minimax_refs`, then returns both CONDITIONING and the matching H3 latent.\n"
            ...
            "image_inputs_and_placeholders": (
                ...
                "### MiniMax H3\n\n"
                "`UC_AdvancedMiniMaxH3ImageToVideo` is the complete H3 image-conditioning path. It flattens image sockets and batches into one visible order. In `first frame` mode the first connected source is the VAE first frame; in `first + last frames` mode the second connected source is also the VAE last frame. Every flattened source participates in native H3 Qwen conditioning. Keyframe modes may spatially fuse per-source native H3 branches; with fusion off they use one native ordered-image presentation.\n\n"
                "In `reference images` mode every flattened source is resized by the visible `ref_image_size` choice, sent as identical pixels to Qwen through native `minimax_ref_items` and to the VAE, and retained as a separate ordered `minimax_refs` entry. `match` limits reference area to the generation canvas; `max` limits the short edge to 2048 pixels; final 32-pixel alignment can marginally enlarge a dimension. Spatial fusion must be disabled in this mode. The size control is ignored by the two keyframe modes. The node returns both CONDITIONING and the matching H3 latent and does not run resolution consensus.\n\n"
