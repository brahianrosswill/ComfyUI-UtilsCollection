    ...
    def execute(cls, topic: str) -> io.NodeOutput:
        topics = {
            "node_catalog": (
                ...
                "- `UC_AdvancedMiniMaxH3ImageToVideo`: complete MiniMax H3 first-frame or first/last-frame conditioning. It spatially fuses every ordered image source for Qwen while assigning VAE keyframes strictly from image positions, then returns both CONDITIONING and the matching H3 latent.\n"
            ...
            "image_inputs_and_placeholders": (
                ...
                "### MiniMax H3\n\n"
                "`UC_AdvancedMiniMaxH3ImageToVideo` is the complete H3 first/last-frame path. It flattens image sockets and batches into one visible order: image 1 is always the VAE first frame, and in `first + last frames` mode image 2 is the VAE last frame. Every flattened image still participates in Qwen conditioning. With fusion enabled they form one fused visual slot; with fusion off they remain separate `<Picture N>` entries. The node returns both CONDITIONING and the matching H3 latent.\n\n"
